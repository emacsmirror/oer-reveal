...
# Macro revealimg() expects up to five arguments, a filename for a
...
# Macro reveallicense() expects up to five arguments, the filename
...
# 7. Optional indicator when grid images should be fragmented (appear
#    one after the other).
